      ...
      compile: recursionHelper(function ($scope, elem, attr, ngModel) {
          ...
          function initScope(type) {
            ...
            if (type === 'Object') {
              ...
              $scope.jsonFilter = function (idx) {
                var item = $scope.value[idx];
                var filter = $scope.parentFilter || '';
                if (filter.length < 1) {
                  return true;
                }
                filter = filter.split('').reduce(function (a, b) {
                  return a + '[^' + b + ']*' + b;
                });
                return (new RegExp(filter, 'i')).test(item.key);
              };
              $scope.parentFilter = '';
              $scope.childFilter = '';
              $scope.$watch('filter', function (filter) {
                var parts = filter.split('.');
                $scope.parentFilter = parts.shift();
                $scope.childFilter = parts.join('.');
              });
            }

            if (type === 'Array') {
              $scope.appendItem = function () {
                $scope.value.push(null);
              };
